...
/*****************************syringe****************************************************************/
 ...
 void ok_syringe(float syringe_dia) //which syringe is detected 60ml,20ml
 {
	 for(int i=1; i<=10; i++)
	 {
...
/************************************syringe check init*******************************************/
 ...
 void syringe_check_init(void) // first check syringe  init
 {
	 if(PORTC.IN & PIN7_bm)
	 {
		 syringe_detected = false;
...
/***********************occlusion check**********************************************************/
 ...
 void Occlusion_check(Avg) // occlusion check 60ml,20ml,10ml,5ml syringe
...
/******************************************loadcell**************************************************/
 ...
 int main(void)
 {
	 ...
	 while(1)
	 {
		 USART1_sendString("niss");
		 syringeforward();
		 //syringe_dia();
		 if (run_ok_syringe)
		 {
			 ok_syringe(syringe_dia);
